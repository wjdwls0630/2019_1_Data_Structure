...
int BaseApplication::UpdateItemName(std::string Target, std::string Value){
	...
	switch (this->CheckValueItemType(Target)) { //check Target First
		...
		switch (this->CheckValueItemType(Value)) {
			...
			if (!(Temp_ItemPtr=this->m_fdCurPointer->RetrieveItemPtrByName(Temp_Item))) { //if not found
				throw ItemNotFound(Target);
			}
			if(this->m_fdCurPointer->UpdateItemName(*Temp_ItemPtr, Value)){
                //set modifed time
                (*Temp_ItemPtr)->GenerateTime(current_time);
                (*Temp_ItemPtr)->SetModifiedTime(current_time);
                //update folder
                return 1;
			}
			return 0;
    ...
    (*Temp_ItemPtr)->SetName(Value); // Update SubFile Name
    (*Temp_ItemPtr)->SetPath(this->m_fdCurPointer->GetPath()+"/"+Value); // Update SubFile Path
