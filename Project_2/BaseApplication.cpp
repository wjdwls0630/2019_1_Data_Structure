...
// for using temp and target and process command
    ...
    while (this->GetCommand(this->m_Command)) {
        ...
        //initialize
        Check_CurPtr = this->m_fdCurPointer;
            ...
            case 2: //cd
                try{
                    ...
                    if (Value=="../") {// go to parent
                        ...
                        // add to cache
                        this->m_Cache.Add(this->m_fdCurPointer)
            ...
            case 8: //copy
                try{
                    if (Option==""){// copy
                        ...
                        //copy the data
                        this->CopyItem(Option); //copy target and store in ClipBoard
...
int BaseApplication::RunFile(std::string Target){
    ...
    if(this->GetTargetItemPtrInPath(Target,Target_ItemPtr)){
        ...
        // add to cache
        this->m_Cache.Add(*Target_ItemPtr)
        return 1;
    }
    return 0;
}
...
int BaseApplication::CopyItem(std::string Target){
    ...
    if (this->GetTargetItemPtrInPath(Target,Target_ItemPtr)) {
        // if ClipBoard has another memory
        if (this->ClipBoard!=nullptr) {
            delete this->ClipBoard;
        }
        if ((*Target_ItemPtr)->WhatIs()=="Folder") {
            this->ClipBoard = new FolderType;
            //copy
            *dynamic_cast<FolderType*>(this->ClipBoard) = *dynamic_cast<FolderType*>(*Target_ItemPtr);
        } else if ((*Target_ItemPtr)->WhatIs()=="TextFile") {
            this->ClipBoard = new TextFileType;
            //copy
            *dynamic_cast<TextFileType*>(this->ClipBoard) = *dynamic_cast<TextFileType*>(*Target_ItemPtr);
        } else if ((*Target_ItemPtr)->WhatIs()=="JPGFile") {
            this->ClipBoard = new JPGFileType;
            //copy
            *dynamic_cast<JPGFileType*>(this->ClipBoard) = *dynamic_cast<JPGFileType*>(*Target_ItemPtr);
        } else if ((*Target_ItemPtr)->WhatIs()=="MP3File") {
            this->ClipBoard = new MP3FileType;
            //copy
            *dynamic_cast<MP3FileType*>(this->ClipBoard) = *dynamic_cast<MP3FileType*>(*Target_ItemPtr);
        }
        return 1;
    }
...
int BaseApplication::Paste(std::string Value){
    if (this->ClipBoard==nullptr) { // if has no data in ClipBoard do nothing
        return 0;
    }
    ...
    if (this->GetTargetItemPtrInPath(Value, Target_ItemPtr)) {
        //paste
        //set ClipBoard new path
        this->ClipBoard->SetPath((*Target_ItemPtr)->GetPath()+"/"+this->ClipBoard->GetName());
        dynamic_cast<FolderType*>(*Target_ItemPtr)->NewItem(this->ClipBoard);
        return 1;
    }
    return 0;
}
...
void BaseApplication::SearchItemsByName(std::string Value){
    ...
    FolderType Retrieve_Folder_Heap;
    CHeap Temp_Cache;
    Temp_Cache = this->m_Cache;
    ItemType* Item_Heap_Ptr;
    for (int i = 0; i < Temp_Cache.GetLength() ; i++) {
        Item_Heap_Ptr = Temp_Cache.Dequeue()
        if (Item_Heap_Ptr->GetName().find(Value) != std::string::npos) {
            Retrieve_Folder_Heap.NewItem(Item_Heap_Ptr)
        }
    }
