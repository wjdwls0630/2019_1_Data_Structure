...
private:
    ...
    ItemType* ClipBoard; //store cut and copy
    CHeap<ItemType*> m_Cache // store cache
public:
    /**
    *	default constructor.
    */
    BaseApplication ():Root("Root"),ClipBoard(nullptr){
    ...
    * @detail operation copy [target] [value]
                        copy [target] store on ClipBoard
    ...
    *	@post	When copy is finished, ClipBoard store the info
    ...
    * @detail operation cut [target] [value]
                        cut [target] store on ClipBoard and delete it
                        cut [target] [value] cut target and paste in value
                        move [target] [value] cut target and paste in value
                        it can be controlled in all child's path
    *	@pre	target can be folder or file but value must be folder path.
    *	@post	When cut is finished, ClipBoard store the info and delete item
    ...
    * @detail operation paste [value]
                        paste [target] paste ClipBoard data to Value
                        default value is current folder
                        it can be controlled in all child's path
    *	@pre	target can be folder or file but value must be folder path.
    *	@post	When paste is finished, add ClipBoard item to value
