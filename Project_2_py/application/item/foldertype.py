...
from application.structure.avl_tree import *
from application.structure.binary_iterator import *
from application.error.empty_folder import *
...
class FolderType(ItemType) :
    ...
    *	@return	return self-reference if self and data is not same object, otherwise self unchanged.
    ...
    def __iadd__(self, other):
        if other.fd_sub_item_num != 0:
            # if fd has sub items
            if self.fd_sub_item_num == 0:
    ...
    def new_item(self, temp_item: ItemType) -> int:
        ...
        while self.fd_sub_item_list.get(temp_item):
            ...
            if temp_item.what_is() == "Folder":
                ...
                temp_item.set_path(self.get_path()+"/"+temp_item.get_name())
                # set sub folder path
            elif temp_item.what_is().find("File") != -1:
                # if file
    ...
    @return	return 1 if this function works well, otherwise 0.
    """
    def delete_item(self, temp_item: ItemType) -> int:
        if self.fd_sub_item_num == 0:
    ...
    @return	return 1 if this function works well, otherwise 0.
    """
    def retrieve_item_by_name(self, target_item: ItemType) -> int:
        if self.fd_sub_item_num == 0:
            raise ItemNotFound(target_item.get_name())

        # If Folder is not in the Current Folder
        retrieve_item = self.fd_sub_item_list.get(target_item)
    ...
    def retrieve_items_by_name(self, keyword: str):
        ...
        while not iter.next_is_tail():
            ...
            if iter.next().get_name().find(keyword) != -1:
                retrieve_folder.new_item(iter.cur())

            if iter.cur().what_is() == "Folder":
                if iter.cur().get_sub_item_num() != 0:
                    #call recursive when catch, end the loop
                    try:
                        retrieve_folder += iter.cur().retrieve_items_by_name(keyword)
    ...
    *	@return	none.
    ...
    def reset_folder_key(self):
        ...
        while not iter.next_is_tail():
            # set all items key
            if iter.next().get_key() != KeyValue.NAME:
...
if __name__ == "__main__":
    item1 = FolderType("aaa")
    item2 = FolderType("bbb")
