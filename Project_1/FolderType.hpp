...
private:
  ...
  SLinkedList<FolderType>* fdSubFolderList; // subfolderlist
  SLinkedList<FileType>* fdSubFileList; // subfolderlist
...
public:
  ...
  : ItemType(fd){
    ...
    if (fd.fdSubFolderNum!=0) {
      ...
      for (int i = 0; i < fd.fdSubFolderNum; i++) {
        fd.fdSubFolderList->GetNextItem(Temp_Folder);
  ...
  */
  int GetSubItemrNum() const { return this->fdSubItemNum; }
