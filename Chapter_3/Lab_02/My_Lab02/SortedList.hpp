...
public:
	...
	*	default constructor.
	*/
	SortedList(int size=MAXSIZE)
	:m_Length(0), m_CurPointer(-1), maxSize(size),m_Direction(ASCENDING),m_Comparer(ASCENDING){
		this->m_Array=new T[this->maxSize];
	}
	/**
	*	m_Direction constructor.
	*/
	SortedList(const int &Direction,int size=MAXSIZE)
	:m_Length(0), m_CurPointer(-1), maxSize(size),m_Direction(Direction),m_Comparer(Direction){
		this->m_Array=new T[this->maxSize];
	...
	*	destructor.
	*/
	~SortedList()	{ delete [] this->m_Array; }

	/**
	*	@brief	set the sorting direction.
	*	@pre	none.
	*	@post	direction is set.
	*/
	void SetDirection(const int &Direction){ this->sortDirection=Direction;}
	...
	*/
	...
	/**
	*	@brief	add a new data(not overlap) into Sortedlist. if list is full, reallocate the list first, then add it into list
	*	@pre	none.
	*	@post	added the new record(not overlap) into the Sortedlist.
	*	@param	data	new data.
	*	@return return 1 if this function works well, otherwise 0.
	*/
	int Add(T data);
...
private:
	...
	int maxSize;		///< iterator pointer.
	int m_Direction; /// ASCENDING(1), DESCENDING(0)
	Comparer<T> m_Comparer; /// set Compare
};
...
int SortedList<T>::GetBinarySearch(T &data){
	...
	while (left<=right) {
		...
		if (this->m_Array[mid]==data) {
			...
			return 1;
		}else if(this->m_Comparer.Compare(this->m_Array[mid],data)){
			left=mid+1;
		} else if(this->m_Comparer.Op_Compare(this->m_Array[mid],data)){
			right =mid-1;
		}
	}
	return 0;
...
int SortedList<T>::Add(T data){
	...
	for (int i = 0; i < this->m_Length; i++) {
		...
		if (this->m_Comparer.Compare(data,Temp_Data)) {
